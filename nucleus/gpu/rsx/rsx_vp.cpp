...
std::string RSXVertexProgram::get_src(U32 n)
{
    std::string src;
...
std::string RSXVertexProgram::get_dst()
{
    std::string dst;
...
void RSXVertexProgram::decompile(rsx_vp_instruction_t* buffer)
{
...
bool RSXVertexProgram::compile()
{
    ...
    if (status != GL_TRUE) {
        ...
        logger.error(LOG_GPU, "RSXVertexProgram::compile: Can't compile shader. Reason:\n%s", infoBuffer);
        return false;
    }
    */
    return true;
